...
# make a base map
#canada <- ne_states(country = "canada", returnclass = "sf") %>% st_transform(3347) #st_transform(4326)
...
events<-left_join(events, map, by=c("RouteIdentifier", "latitude", "longitude"))
...
events<-events %>% select(SiteCode, RouteIdentifier, survey_year, latitude, longitude, Easting, Northing, bcr)
...
tau_route<-data.frame(
  ...
  tau_ll=tau_lci$tau_ll,
  tau_uci=tau_uci$tau_ul,
...
tau_route$precision_cat<-ifelse(tau_route$tau_iw<3.5, "High", ifelse(tau_route$tau_iw>=3.5 & tau_route$tau_iw<=6.7, "Medium", "Low"))
