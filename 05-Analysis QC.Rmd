...
sp.data <- sp.data %>% mutate(site_idx = factor(paste(RouteIdentifier))) %>% 
  ...
  mutate(
    ...
    site_year_idx = as.numeric(factor(paste(RouteIdentifier, survey_year)))) %>%
...
A1<-inla.spde.make.A(mesh2, loc=Loc)
dim(A1)
...
##-----------------------------------------------------------
##Group Matrix for each year of data
#group<-sp.data$survey_year-min(sp.data$survey_year)
#ngroup=length(unique(group)) #15 years of data = 15 groups
  
#make predictor matrix for the mesh model
#Loc2<-sp.data %>% select(longitude, latitude) %>% 
#    st_drop_geometry() %>% as.matrix()

#A1<-inla.spde.make.A(mesh2, loc=Loc2, group=group)
#dim(A1)

#mesh3<-inla.mesh.2d(Loc2, 
#  boundary = hull,
#  max.edge = c(50, 200), # km inside and outside
#  cutoff = 0,
#  crs = fm_crs(sp.data))
...
#A1<-inla.spde.make.A(mesh3, loc=Loc2, group=group)
#dim(A1)
