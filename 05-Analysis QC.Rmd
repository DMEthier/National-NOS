...
for (i in 1:n_samples) {
  ...
  random_effects_alpha <- posterior_samples[[i]]$latent[2:(n_cells + 1)]  # ICAR random intercepts
  random_effects_tau <- posterior_samples[[i]]$latent[(n_cells + 2):(2 * n_cells + 1)]  # ICAR random year slopes
  random_effects_kappa <- posterior_samples[[i]]$latent[(2 * n_cells + 2):(2 * n_cells + 1 + n_routes)]  # IID random route intercepts

 # Calculate predicted counts for each cell
  # Here, we assume that std_yr is a vector of year values for each cell
  predicted_counts[i, ] <- exp(fixed_effects + random_effects_alpha + random_effects_tau * sp.data$survey_year + random_effects_kappa)
}
  
##HERE
# Calculate the mean predicted counts
mean_predicted_counts <- rowMeans(predicted_counts)

# Estimate the probability of decrease for each cell
probability_decrease <- apply(predicted_counts, 2, function(x) mean(x < mean_predicted_counts))

# View the probabilities
print(probability_decrease)
