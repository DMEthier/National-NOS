...
Now that the data have been cleaned we are ready (almost) for the trend analysis. The analysis can be done at multiple spatail scales and using 1 of 2 response varaible. Here we will focus using `ProtocolCode` and `ObservationCount` first. 

ObservationCount = Total
ObservationCount1 = 1st minute + 2nd minute

Code to read and append the relevant files from the out directory. This will be needed for the ObservationCount1 analysis when we are looking at national trends. 
...
obs <- obs %>% filter (CommonName %in% c("Barred Owl", "Boreal Owl", "Great Gray Owl", "Great Horned Owl", "Northern Saw-whet Owl", "Eastern Screeh-Owl", "Western Screech-Owl", "Long-eared Owl", "Short-eared Owl", "Northern Hawk Owl", "Northern Pygmy-Owl", "Snowy Owl", "Barn Owl", "Flammulated Owl") | is.na(CommonName))
...
obs<-obs %>% select("CommonName", "species_id", "RouteIdentifier", "SiteCode", "collection", "ProtocolCode", "survey_day", "survey_month", "survey_year", "CollectorNumber", "StateProvince", "bcr", "ObservationCount", "ObservationCount1", "date", "doy")

#zero fill by merging with the events dataframe. 
test <- left_join(event.data, obs, by = c("RouteIdentifier", "survey_year", "CollectorNumber")) %>% mutate(ObservationCount = replace(ObservationCount, is.na(ObservationCount), 0),                                                        ObservationCount1 = replace(ObservationCount1, is.na(ObservationCount1), 0))

#some observation are lost. Assume those missing lat long at route starts. 
