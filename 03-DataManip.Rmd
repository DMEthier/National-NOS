...
dat<-dat %>% separate(SamplingEventIdentifier, c("del1", "del2", "StopNo"), sep="-", remove=FALSE) %>% 
  dplyr::select (-del1, -del2) %>% mutate(SiteCode= paste(RouteIdentifier, StopNo, sep="-")) %>% select(-StopNo)
}
...
loc.dat<-loc.dat %>% filter(Stop==1) %>% filter (!is.na(latitude), !is.na(longitude)) %>% select(RouteIdentifier, Stop, latitude, longitude, StateProvince, bcr) %>% distinct(RouteIdentifier, .keep_all = TRUE) %>% select(-Stop)

write.table(loc.dat, paste(out.dir, "Location.csv"), row.names = FALSE, append = TRUE, quote = FALSE, sep = ",", col.names = TRUE)

##____________________________________________________________________
#Because in the early years the Ontario owls program ran upwards of 4 surveys/route/year, the duplicates will need to be removed. We run this script for all province since there are some other doublicate routes that have snuck into the assessment. 
...
test <- test %>% group_by(RouteIdentifier, survey_year)%>%
  summarize(sample_events=n_distinct(survey_day))

#merge back with main dataframe 
dat<-left_join(dat, test, by= c("RouteIdentifier", "survey_year"))
...
minday<-subowls %>% group_by(RouteIdentifier, survey_year)%>%
  summarize(minday=min(doy, na.rm=TRUE))

#merge back with main data, eliminate the unwanted data, select needed columns
dat<-left_join(dat, minday, by= c("RouteIdentifier", "survey_year"))
...
event.data <- dat %>%
  select(RouteIdentifier, survey_year, CollectorNumber, nstop, StateProvince, bcr) %>%  
  distinct() %>%
  ungroup() %>%
  as.data.frame()

#merge with the loc.data to assign unique lat long to each route
event.data<-left_join(event.data, loc.dat, by=c("RouteIdentifier", "StateProvince", "bcr"))
...
dat<-dat %>% select(-ObservationCount2, -ObservationDescriptor3)
...
#Species must be detected with a mean of ~3 per years to be included in the analysis. 
