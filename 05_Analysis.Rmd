...
#inla.formula1 <- count ~ cyear + croute + f(RouteIdentifier, model = #"iid") + f(yearfac, model = "ar1") + (id.iid, model="iid")

#Overdispersed Poisson with no spatail autocorrelation effect
# trend.op<-NULL
# trend.op<-inla(inla.formula1, family = "poisson", data = #data.frame(sp.data, id.iid = 1:nrow(sp.data)), control.predictor = #list(compute=TRUE), control.compute = list(config=TRUE, dic=TRUE))
...
inla.formula <- count ~ cyear + croute +   
...
#rename columns
myData<-rename(myData, c("mean"="mu", 
                         "0.025quant"="selo", 
                         "0.975quant"="seup"))
...
pi.inla<-trend_lcb$summary.hyper[1,"mean"]  
mu<-trend_lcb$summary.fitted.values[,"mean"]
p<-length(trend_lcb$names.fixed)+1
ExpY<-(1-pi.inla)*mu
...
inla.formula2<-formula(count ~ 1 + x1 + x2 + x3 + croute +   
  f(RouteIdentifier, model = "iid") + f(yearfac, model = "ar1"))
...
#I am not positive this is correct here. 
#pred.marg.cr<-trend_cr$marginals.lincomb.derived
#myData.cr$mu<-unlist(lapply(pred.marg.cr, function(x) #inla.emarginal(exp,x)))
#myData.cr$selo<-unlist(lapply(pred.marg.cr, function(x) #inla.qmarginal(c(0.025), inla.tmarginal(exp,x))))
#myData.cr$seup<-unlist(lapply(pred.marg.cr, function(x) #inla.qmarginal(c(0.975), inla.tmarginal(exp,x))))
#myData.cr<-distinct(myData.cr)
...
ggplot()+
  ...
  theme_light()+
  theme(text=element_text(size=15))+
  geom_line(data=myData, aes(x=year, y=mu), colour="blue")+
  geom_ribbon(data=myData, aes(x=year, ymax=seup, ymin=selo), alpha=0.3)+
  geom_line(data=myData.cr, aes(x=year, y=mu), colour="red")+
  geom_ribbon(data=myData.cr, aes(x=year, ymax=seup, ymin=selo), alpha=0.3)
